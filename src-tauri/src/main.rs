...
#[tauri::command]
async fn check_process() -> String {
    let cmd: Result<std::process::Output, std::io::Error> =
            std::process::Command::new("wmic")
                    .creation_flags(0x08000000)
                    .args(["process","where","\"name=\'crostouchscreen.2.9.4-installer-x86_64-pc-windows-msvc.exe\'\"","get","Captions"])
                    .output();
    
            let output: String = match cmd {
                Ok(output) => String::from_utf8_lossy(&output.stdout).split("\n").map(|x| x.to_string()).collect::<Vec<String>>()[0].clone(),
                Err(e) => {
                    println!("boardnameError `{}`.", e);
                    String::from("deeznuts") // This match returns a blank string.
                }
            };
            let output2 = output.trim();
            return String::from(output2);
        }